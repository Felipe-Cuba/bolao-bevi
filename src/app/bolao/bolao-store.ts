...
import { BolaoEntry, BOLAO_STORAGE_KEY, newEntryId, Palpite } from './bolao.types';
import { groupEntries$, saveEntryApi, removeEntryApi } from './grupo-firestore';
...
export class BolaoStore {
  ...
  private readonly groupEntries = toSignal(
    toObservable(this.groupId).pipe(
      switchMap((id) =>
        id ? groupEntries$(this.injector, this.firestore, id) : of([] as BolaoEntry[]),
      ),
    ),
  ...
  async createEntry(name: string): Promise<BolaoEntry> {
    ...
    if (gid) {
      const { entry } = await saveEntryApi(gid, clean, []);
      return entry;
    }
  ...
  async setPalpites(id: string, palpites: Palpite[]): Promise<void> {
    ...
    if (gid) {
      ...
      await saveEntryApi(gid, entry?.name ?? 'Sem nome', palpites, id);
      return;
    }
  ...
  async importEntries(incoming: BolaoEntry[]): Promise<BolaoEntry[]> {
    ...
    if (gid) {
      ...
      for (const entry of incoming) {
        ...
        taken.add(name);
        const { entry: saved } = await saveEntryApi(gid, name, entry.palpites);
        added.push(saved);
      }
      return added;
    }
