...
import { GroupStanding } from './wc-derivations';
            ...
            <tbody>
              @for (row of g.rows; track row.team.id; let i = $index) {
                <tr [class.qualify]="i < 2">
                  <td class="table__pos">{{ i + 1 }}</td>
                  <td class="table__team">
                    <img class="table__crest" [src]="teamCrest(row.team)" [alt]="teamName(row.team)" loading="lazy" />
